...
from models.enf.bi_invariants import TranslationBI
from models.enf.utils import create_coordinate_grid, initialize_latents
from models.downstream.enf_transformer import TransformerClassifier
...
def get_config():
    ...
    config.recon_enf.num_out = 3                    # 3 Channels out for RGB
    ...
    config.dataset.num_signals_train = 100           # Number of training signals (-1 for all)
...
def main(_):
    ...
    # Initialize wandb
    run = wandb.init(project="enf-hack", config=config.to_dict(), mode="online" if not config.debug else "dryrun")
    ...
    x = create_coordinate_grid(batch_size=config.dataset.batch_size, img_shape=img_shape)
